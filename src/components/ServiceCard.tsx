...
export function ServiceCard({ serviceName, ...props }) {
    const navigate = useNavigate();
    const handleGetQuote = () => {
        navigate(`/order?service=${encodeURIComponent(serviceName)}`);
    };

    return (
        <button
            onClick={handleGetQuote}
            className="bg-[#c9a227] text-white px-4 py-2 rounded hover:bg-[#b08e1e] transition"
        >
            Get Quote
        </button>
    );
